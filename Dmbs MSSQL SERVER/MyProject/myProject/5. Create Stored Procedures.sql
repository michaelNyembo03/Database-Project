...
IF NOT EXISTS ( SELECT playerEmail FROM player WHERE playerEmail = @player_Email)
      BEGIN
	       PRINT'Invalid player email, choose an existing one or register a new player before continue!'
      END
...
BEGIN
	  ...
	  PRINT ' '
	  PRINT 'We have emailed this letter to ' + (@player_Email) + SPACE(1) + 'informing you that you have ' + CAST(@accountTime AS VARCHAR) + SPACE(1) + 'Days' + SPACE(1) + 'left'
